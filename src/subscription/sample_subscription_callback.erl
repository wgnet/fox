...
-behaviour(fox_subscription_worker).
...
init(Channel, Args) ->
    error_logger:info_msg("sample_subscription_callback:init channel:~p args:~p", [Channel, Args]),
...
terminate(Channel, State) ->
    error_logger:info_msg("sample_subscription_callback:terminate channel:~p, state:~p", [Channel, State]),
